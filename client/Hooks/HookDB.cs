    ...
    public static class HookDB {
        public static Dictionary<string, HookEntry> hooks = new Dictionary<string, HookEntry> {
            ...
            { "getRounds", new HookEntry(
                ...
                new byte[7] { 0x8B, 0x81, 0x48, 0x04, 0x00, 0x00, 0xC3 })
            },
            {"getTimer", new HookEntry(
                new Hooks.DGetTimer(Hooks.GetTimerHook),
                Hooks.GetTimerAddress,
                new byte[7] { 0x8B, 0x81, 0x4C, 0x04, 0x00, 0x00, 0xC3 })
            },
            { "selectMenu", new HookEntry(
                 new Hooks.DSelectMenu(Hooks.SelectMenuHook),
                 Hooks.SelectMenuAddress,
                 new byte[64] { 0x48, 0x89, 0x5C, 0x24, 0x08, 0x57, 0x48, 0x83, 0xEC, 0x20, 0x48, 0x8B, 0xF9, 0xE8, 0xCE, 0x8C,
                                0xC9, 0xFF, 0x48, 0x8D, 0x97, 0x78, 0x04, 0x00, 0x00, 0x48, 0x8B, 0xC8, 0xE8, 0x4F, 0x4F, 0xCA,
                                0xFF, 0xE8, 0x3A, 0xD4, 0xDA, 0xFF, 0x48, 0x8B, 0xC8, 0xE8, 0xF2, 0xCB, 0xDA, 0xFF, 0x48, 0x85,
                                0xC0, 0x74, 0x09, 0x8B, 0x8F, 0x44, 0x04, 0x00, 0x00, 0x89, 0x48, 0x58, 0x48, 0x8B, 0x5C, 0x24 })
            },
            ...
            { "setupGameUI", new HookEntry(
                ...
                new byte[32] { 0x48, 0x8B, 0xC4, 0x48, 0x89, 0x58, 0x10, 0x48, 0x89, 0x70, 0x18, 0x48, 0x89, 0x78, 0x20, 0x55,
                               0x41, 0x54, 0x41, 0x55, 0x41, 0x56, 0x41, 0x57, 0x48, 0x8D, 0xA8, 0xB8, 0xF8, 0xFF, 0xFF, 0x48 })
            }
        };
    }
}